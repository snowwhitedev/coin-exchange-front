...
export const mutations = {
  ...
  SET_FILTERED_SYMBOLS(state, payload) {
    if (payload === 'USDT') {
      const res = _.filter(state.Symbols, function (o) { return o.QuatoAsset === payload })
      state.filteredSymbols = res
      console.log('[res]', res)
    } else {
      const res = _.filter(state.Symbols, function (o) { return o.BaseAsset === payload })
      state.filteredSymbols = res
    }
...
export const actions = {
  async FECTCH_SYMBOLS(context) {
    const authHeaders = { Authorization: 'Bearer ' + this.$cookies.get('Token') }
    const response = await this.$axios.get(
      this.$config.FINANCE_API_URL + `/GetSymbols`, { headers: authHeaders }
    )
  ...
  async SET_SELECTED_ASSET(context, payload) {
    ...
    if (networks && networks.length > 0) {
      ...
      for (const item of networks) {
        const responseUserDepositAddress =
          await this.$axios.get(this.$config.FINANCE_API_URL + `/GetUserDepositAddress?Asset=${context.getters.SELECTED_ASSET}&Network=${item['Network']}`, {
            headers: authHeaders
          })
  ...
  async POST_WITHDRAW(context, payload) {
    try {
      ...
      console.log(`Error occurred, when posting withdraw request: ${e}`)
      return false;
    }
  },
}
