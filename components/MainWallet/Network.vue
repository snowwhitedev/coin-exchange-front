<template>
  ...
  >
    <a-col style="margin-top: 30px">
      ...
      <div v-else-if="networkType === 'withdraw'">
        <a-spin :spinning="IS_NETWORK_READY">
          <a-col offset="1" :span="22" style="color: #1c2640">{{
            networkTitle
          }}</a-col>
                          ...
                          v-decorator="[
                            'address',
                            {
                              rules: [
                                {
                                  required: true,
                                  message: 'Lütfen geçerli bir adres girin',
                                },
                                {
...
export default {
  ...
  methods: {
    ...
    validateAddress(rule, value, callback) {
      ...
      if (value === '') {
        callback(new Error('Lütfen geçerli bir adres girin'))
    ...
    async postWithdraw(network) {
      ...
      if (this.form.address !== null && this.form.amount !== null) {
        ...
        try {
          ...
          if (response) {
            ...
            if (response.data.Success) {
              ...
              this.setVerficationData({
                ...
                status: true,
                redirectURI: '/my/wallet/main-wallet/withdraw',
              })
          ...
          this.SET_IS_NETWORK_READY(false)
          console.log(`Error occurred, when posting withdraw: ${e}`)
        }
      }
    },
  },
