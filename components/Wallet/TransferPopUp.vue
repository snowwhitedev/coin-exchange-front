<template>
  <div class="">
    ...
    <a-form-model :model="TransferData" :rules="TransferDataRules" ref="TransferForm">
...
export default {
  ...
  data() {
    const validValue = (rule, value, callback) => {
      if (!!value) {
        ...
        callback(new Error('Must be filled !'))
      }
    }
    const AmountCheck = (rule, value, callback) => {
      if (value <= this.TransferAssetAvailableAmount) {
        ...
        callback(new Error('Amount Must be Less than Total Available Balance !'))
      }
    }
    return {
      TransferWallets: [],
      FIAT_WALLETS : ['Fiat Wallet', 'P2P Wallet'],
      UserWalletsBalance: [],
      TransferData: {
        From: 'Main Wallet',
        To: 'Fiat Wallet',
    ...
    async getTransferWallets() {
      try {
        ...
        if (data.Success) {
          ...
          this.TransferWallets = TransferList.filter(({ Key }) => Key !== 'Security Wallet')
        }
      } catch (e) {
        e.response ? console.log(e.response.data) : console.error(e)
      } finally {
        this.isTransferPopBusy = false
      }
    },
  ...
  computed: {
    TransferWalletsFrom () {
      return this.TransferWallets.filter(item => item.Key !== this.TransferData.To)
    },
    TransferWalletsTo () {
      return this.TransferWallets.filter(item => item.Key !== this.TransferData.From)
    },

    // Active Wallet Assets
    WalletAssets () {
      if (this.TransferData.From === 'Fiat Wallet' || this.TransferData.To === 'Fiat Wallet') {
        return this.UserWalletsBalance.UserFiatBalance
      }
      if (this.TransferData.From === 'P2P Wallet' || this.TransferData.To === 'P2P Wallet') {
    ...
    TransferAssetAvailableAmount () {
      if (this.WalletAssets) {
        let wa = Object.values(this.WalletAssets).filter(wa => wa.asset === this.TransferData.Asset)
