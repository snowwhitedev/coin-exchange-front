<template>
  <div class="Roboto-Regular">
    <a-col offset="2">
      <div class="p2p-history-title">
        Tüm P2P İşlem Geçmişi
      </div>
    </a-col>
    ...
    <a-col class="Roboto-Bold" offset="2" style="margin-top: 20px">
      <a-table :columns="columns" :data-source="data" class="components-table-demo-nested">
        <span slot="Type" slot-scope="Type, record"
              :style="Type === 'BUY' ? 'color: #5CB56A' : 'color: #FD0D1B'">{{ Type === 'BUY' ? 'Alıcı' : 'Satıcı'
          }}</span>
        <template v-slot:expandedRowRender="record, index, indent, expanded">
          <a-table :data-source="record.OTC_RequestList"
                   :pagination="false">
            ...
            <a-table-column title="Action" dataIndex="operation" key="operation"
...
<script>
const columns = [
  { title: 'Satıcı/Alıcı', dataIndex: 'Type', scopedSlots: { customRender: 'Type' } },
  ...
  { title: 'Talem Sayısı', dataIndex: 'OpenRequestNumber', key: 'OpenRequestNumber' },
  { title: 'İlan Numarası', dataIndex: 'OTC_OrderUniqueID', key: 'OTC_OrderUniqueID' }
]
  ...
  methods: {
    async getP2POrders() {
      ...
      const response = await $axios.get(
        $config.FINANCE_API_URL + '/GetP2PMyOpenOrders', { headers: authHeader })
