<template>
  <div class="Roboto-Regular">
    ...
    <a-breadcrumb class="p2p-breadcrumb">
      ...
      <a-breadcrumb-item style="color: white">{{ breadcrumb }}</a-breadcrumb-item>
    </a-breadcrumb>
    ...
    <a-col class="p2p-main-tab" offset="2" :span="10">
      <a-menu v-model="current" class="p2p-menu" mode="horizontal">
        <a-menu-item key="Market" @click="menuChanged">{{ $t('p2p_market') }}</a-menu-item>
        <a-menu-item key="GivenOrders">{{ $t('place_order') }}</a-menu-item>
        <a-menu-item key="History" @click="menuChanged">{{ $t('p2p_history') }}</a-menu-item>
      </a-menu>
    </a-col>

    <a-col class="p2p-another-tab" offset="12" :span="11" align="right">
      <a-menu v-model="current" class="p2p-menu" mode="horizontal">
        <a-menu-item key="Settings"><a-icon type="setting" />{{ $t('payment_settings') }}</a-menu-item>
        <a-menu-item key="Orders"><a-icon type="setting" />{{ $t('p2p_orders') }}</a-menu-item>
        <a-button class="ant-btn-primary">{{ $t('p2p_place_new_order') }}</a-button>
      </a-menu>
    </a-col>
