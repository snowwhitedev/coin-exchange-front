    ...
    <Index>
      <template slot="auth-card-content">
        <div class="security-verify-header text-center">
          <h2 class="Roboto-Medium">{{ $t('verify_header') }}</h2>
          <p class="verify-text-secondary Roboto-Regular">
            {{ $t('verify_text_security_verification') }}
          </p>
        </div>
        <a-alert
        ...
        <div v-if="subject !== 'TWOFATOKEN'">
          <div class="my-10" v-if="vrfTypes.includes('sms')">
            ...
            <a-form-model
              class="verify-form-model"
              v-on:submit.prevent="submitOnEnter"
            >
              <a-row :gutter="8">
                <a-col :span="16">
...
export default {
  ...
  mounted() {
    ...
    const { sub, phn, cst, vrf, oem, oph } = jwt_decode(
      SecurityVerificationToken
    )
    this.subject = sub
    this.phn = phn
